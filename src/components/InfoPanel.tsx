...
// --- IMAGENS (ASSUMINDO QUE JÁ ESTÃO IMPORTADAS E MAPEADAS) ---
...
const InfoPanel: React.FC<any> = ({ currentEvent, selectedNode, nodes, canAdvance, onAdvanceDay, isFinalDay }) => {
    ...
    <div className="h-full flex flex-col">
      <div className="overflow-y-auto pr-2 flex-grow">
        <div className="space-y-4">
          ...
          {displayNode && (
            <div className="space-y-4">
              {displayNode.image && imageMap[displayNode.image] && <img src={imageMap[displayNode.image]} alt={displayNode.name} className="w-full h-32 object-cover rounded-lg border border-cyan-500/20" />}
        ...
        {canAdvance ? (
          ...
          <div className="bg-black/20 rounded-lg p-3 border border-cyan-500/10">
            <p className="text-xs text-gray-400 text-center">
              {isFinalDay ? 'Você chegou ao fim da crise. Teste seu conhecimento!' : 'Navegue pela timeline para revisitar eventos passados.'}
            </p>
          </div>
        )}
      </div>
