...
// Estilos de raridade mais detalhados para controlar tudo
const rarityStyles: { [key: string]: { base: string; glow: string; bg: string; text: string; shadow: string; } } = {
  "Comum":    { base: "border-gray-500", glow: "glow-gray", bg: "bg-gray-600", text: "text-gray-300", shadow: "shadow-gray-500/20" },
  "Raro":     { base: "border-blue-500", glow: "glow-blue", bg: "bg-blue-600", text: "text-blue-300", shadow: "shadow-blue-500/20" },
  "Épico":    { base: "border-purple-500", glow: "glow-purple", bg: "bg-purple-600", text: "text-purple-300", shadow: "shadow-purple-500/20" },
  "Lendário": { base: "border-yellow-500", glow: "glow-yellow", bg: "bg-yellow-600", text: "text-yellow-300", shadow: "shadow-yellow-500/20" },
};

// Componente para as partículas da explosão
...
const Lootbox: React.FC<LootboxProps> = ({ figure, onCollect }) => {
  ...
  useEffect(() => {
    ...
    const revealTimer = setTimeout(() => setAnimationState('revealing'), 2700); // 200ms para o flash
    return () => {
              ...
              <Card className={cn("bg-slate-900/95 text-center max-w-sm w-full shadow-2xl border relative", styles.base, styles.shadow)}>
                <div className="p-6">
                  ...
                  <motion.img
                    ...
                    alt={figure.name}
                    className={cn("w-32 h-32 object-cover rounded-full mx-auto my-4 border-4", styles.base)}
                  />
                  
                  <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
                    <h3 className="text-3xl font-bold text-white">{figure.name}</h3>
                    <Badge variant="outline" className={cn("mt-2", styles.base, styles.text, styles.bg, figure.rarity === 'Lendário' && 'animate-pulse')}>{figure.rarity}</Badge>
