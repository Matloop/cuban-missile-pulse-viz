...
interface IndexProps {
  ...
  onSpendToken: () => void;
  onStartQuiz: () => void;
  isTutorialActive: boolean; // Prop recebida do App.tsx
}
...
const Index: React.FC<IndexProps> = ({ 
  ...
  // --- FUNÇÕES DE LÓGICA ---
  const handleResetGame = useCallback(() => { if (window.confirm("...")) { localStorage.removeItem(GAME_STATE_KEY); localStorage.removeItem('cubanCrisisAppState'); localStorage.removeItem(CORRECTLY_ANSWERED_QUIZ_KEY); window.location.reload(); } }, []);
  const handleCutsceneComplete = useCallback(() => { toast.info("..."); setTimeout(() => { localStorage.removeItem(GAME_STATE_KEY); localStorage.removeItem('cubanCrisisAppState'); localStorage.removeItem(CORRECTLY_ANSWERED_QUIZ_KEY); window.location.reload(); }, 2000); }, []);
  const handleBossMinigameWin = useCallback(() => { setShowBossMinigame(false); toast.success("..."); setTimeout(() => { setShowCutscene(true); }, 1500); }, []);
  const handleDateChange = useCallback((newDate: string) => { if (!isTutorialActive) { const newEventIndex = sortedCrisisDates.indexOf(newDate); if (newEventIndex !== -1 && newEventIndex <= highestUnlockedLevel) { setSelectedDate(newDate); } } }, [highestUnlockedLevel, isTutorialActive]);
  ...
  const handleNodeSelect = useCallback((node: NetworkNode | null) => { if (!isTutorialActive) setSelectedNode(node); }, [isTutorialActive]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-screen w-screen overflow-hidden text-white flex flex-col">
      ...
      <header className="bg-black/30 backdrop-blur-sm border-b border-cyan-500/30 p-3 shrink-0 z-20 relative">
        <div className="max-w-screen-2xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            {!isTutorialActive && <RiskIndicator riskLevel={currentEventData ? currentEventData.riskLevel : 1} />}
          </div>
          <div className="flex items-center gap-2">
            <Button onClick={handleStartDailyQuiz} disabled={isTutorialActive} className="bg-gray-700 hover:bg-gray-600">
              <BookOpen className="w-4 h-4 mr-2" /> Quiz
            </Button>
            <Button 
              onClick={handleOpenLootbox} 
              disabled={lootboxTokens <= 0} 
              className={cn("bg-yellow-600 hover:bg-yellow-700", isTutorialActive && "animate-pulse shadow-lg shadow-yellow-500/50 scale-110 z-40 relative")}
            >
              <Key className="w-4 h-4 mr-2" /> Cofre ({lootboxTokens})
            </Button>
            <Button onClick={() => setShowCollection(true)} disabled={isTutorialActive} className="bg-gray-700 hover:bg-gray-600">
              <FolderOpen className="w-4 h-4 mr-2" /> Coleção
            </Button>
            {process.env.NODE_ENV === 'development' && (
              <>
                <Button onClick={handleJumpToLastDay} disabled={isTutorialActive} className="bg-purple-600 hover:bg-purple-700"> Pular </Button>
                <Button onClick={handleResetGame} variant="destructive"> Resetar </Button>
              </>
            )}
          </div>
        </div>
      ...
      <div className={cn("flex flex-col flex-grow min-h-0", isTutorialActive && "opacity-20 pointer-events-none")}>
      ...
      <AnimatePresence>
        ...
        {showCutscene && ( <Cutscene onComplete={handleCutsceneComplete} /> )}
      </AnimatePresence>
    </motion.div>
  );
};
